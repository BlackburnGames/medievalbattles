...
- **`tests/known-issues.txt`, `tests/register-globals.txt`, `tests/broken-queries.txt`, `tests/sql-injection.txt` and `tests/xss.txt` may only ever shrink.** They are ratchets, and the last four double as Phase 3 bug lists. `sql-injection.txt` and `xss.txt` are regenerated with `php tests/sql-injection-audit.php --accept` and `php tests/xss-audit.php --accept`. Both are **empty**, and they stay that way only if new queries go through `mb_sql_str()` / `mb_sql_int()` and new output goes through `mb_h()` / `mb_attr()` — note both audits are per-file and first-order, so moving a query or an echo into an include away from its `mb_input()` clears the entry without fixing anything, and neither sees a value that went through the database on the way.
