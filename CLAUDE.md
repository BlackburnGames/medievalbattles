...
- **`tests/known-issues.txt`, `tests/register-globals.txt`, `tests/broken-queries.txt`, `tests/sql-injection.txt`, `tests/xss.txt` and `tests/xss-stored.txt` may only ever shrink.** They are ratchets, and the last five double as Phase 3 bug lists. Regenerate with `--accept` on the matching script. `sql-injection.txt` and `xss.txt` are **empty**, and they stay that way only if new queries go through `mb_sql_str()` / `mb_sql_int()` and new output goes through `mb_h()` / `mb_attr()`. Both are per-file and first-order, so moving a query or an echo into an include away from its `mb_input()` clears the entry without fixing anything, and neither sees a value that went through the database on the way — `php tests/xss-audit.php --stored` is the list that does, and it is **not** empty.
