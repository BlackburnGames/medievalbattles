...
The work in progress is a staged modernization. Phase 1 (a running, test-covered baseline on PHP 5.6 in Docker) is done. Phase 2, the mechanical port, now **runs on PHP 8.3** — the sources are off `mysql_*`, bare array keys, bare word constants and `ereg_replace`, and the full suite is green on both 8.3 and 5.6 against the same golden master. What is left of Phase 2 is `session_register()` and `register_globals`, both of which need call-site changes rather than renames. Phase 3 is the structural and security rewrite.
...
`docker/php/compat.php` is what it loads. It now supplies only the `session_register()` family, `mb_db_result()`, `mb_client_hostname()`, and a `register_globals` emulation gated behind `MB_REGISTER_GLOBALS=1`. The `mysql_db_query()`, `mysql_result()` and `ereg_replace()` sections were retired in Phase 2 when their call sites were converted.
