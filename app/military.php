...
$urecruit = $_POST['urecruit'];
$usage = $_POST['usage'];
$uthief = $_POST['uthief'];
$uexplorer = $_POST['uexplorer'];
$uwarrior = $_POST['uwarrior'];
$uwizard = $_POST['uwizard'];
$upriest = $_POST['upriest'];
$uarcher = $_POST['uarcher'];
...
if(!IsSet($trains))  {
  ...
  elseif($uthief < 0 OR $usage < 0 OR $uexplorer < 0 OR $uwarrior < 0 OR $uwizard < 0 OR $upriest < 0 OR $uarcher < 0 OR $ucatapult < 0 OR $usuicide < 0)  {
    echo"<div align=center><font class=yellow>You can't train negative or no units!</font></div>";
    include("include/S_RSET.php");
    die();
  }
  elseif($recruits < $uthief + $usage + $uexplorer + $upriest + $uwarrior + $uwizard + $uarcher + $ucatapult + $usuicide) {
    echo"<div align=center><font class=yellow>You don't have that many recruits!</font></div>";
    include("include/S_RSET.php");
    die();
  }
  elseif($gp < ($uthief * 200) + ($uexplorer * $explorec) + ($usage * 500) + ($priestc * $upriest) + ($wizardc * $uwizard) + ($warriorc * $uwarrior) + ($archerc * $uarcher) + ($ucatapult * 5000) + ($usuicide * 5000))   {
  ...
  elseif($totalunits >= $barrack_cap) {
    if($uwarrior > 0 OR $uwizard > 0 OR $upriest > 0 OR $uarcher > 0) {
      ...
      $gp = $gp - ( (($uthief * 200) + ($usage * 500) + ($uexplorer * $explorec)) + (($priestc * $upriest) + ($wizardc * $uwizard) + ($warriorc * $uwarrior) + ($archerc * $uarcher)) + (($ucatapult * 5000) + ($usuicide * $suicidec)));
      $gp = round($gp);

      $lumber = $lumber - ($ucatapult * 200);
      $lumber = round($lumber);

        $recruits = $recruits - ($uexplorer + $uthief + $usage + $upriest + $uwizard + $uwarrior + $uarcher + $ucatapult + $usuicide);
